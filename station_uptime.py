...
Reads an input file with two sections:
[Stations]
<Station ID> <Charger ID> <Charger ID> ...

[Charger Availability Reports]
<Charger ID> <start time nanos> <end time nanos> <up (true/false)>

Outputs to stdout lines:
<Station ID> <uptime percent as integer [0-100] rounded down>

Assumptions / clarifications implemented:
- A station's evaluation window is the inclusive span from the minimum start
  time to the maximum end time among *all availability reports of chargers at that station*.
  (I.e., per-station time window, not global.)
- A gap in a charger's reports counts as downtime (there is no implied "up" outside reported intervals).
- Station uptime is the fraction of that per-station window during which
  *at least one charger at the station reports up=true*.
- If a station has no reports at all (no chargers or no charger entries),
  its uptime is defined as 0 (to avoid division by zero).
- Overlapping and out-of-order intervals are handled robustly. Up intervals are unioned.
- Inputs are expected to be reasonably well-formed per the prompt; lines may contain extra spaces.
- Times are treated as integers (nanoseconds), with end-exclusive semantics for durations
  (duration = end - start). If start >= end, the interval contributes 0.
"""
...
def parse_input(path: str):
    ...
    while i < len(lines):
        ...
        if state == 1:
            ...
            # lines before [Stations] are ignored
            continue

    return station_to_chargers, charger_to_station, reports

def merge_intervals(intervals: List[Tuple[int,int]]) -> List[Tuple[int,int]]:
    """Merge overlapping or touching [start,end) intervals where start<end."""
    if not intervals:
        return []
...
def clipped_total(intervals: List[Tuple[int,int]], clip_start: int, clip_end: int) -> int:
    """Sum of lengths of intervals intersected with [clip_start, clip_end)."""
    total = 0
